...
function isTodayMyBirthday (birthdayDate) {
  const todaysDate = new Date();
  let chosenDate = new Date(birthdayDate);

  const currentDay = todaysDate.getDate();
  const currentMonth = todaysDate.getMonth();

  let birthdayDay = chosenDate.getDate();
  let birthdayMonth = chosenDate.getMonth();
...
function onlyInLastMillenium (arr) {
  return arr.filter(movie => {
    return movie.Year > 999 && movie.Year < 1999}) 
...
function selectId () {
  let selectedElement = document.getElementById("container");
...
function selectByTag () {
  ...
  let selectedTags = document.getElementsByTagName("td"); //or document.querySelectorAll("td")
  return selectedTags;
}
...
function printContent () {
  let selectedTags = document.getElementsByTagName("td");

  for (let i=0; i < selectedTags.length;i++) { 
    console.log(selectedTags[i].innerText) 
  } 
}
...
function addClass () {
  ...
  trTags.forEach(tr => tr.classList.add("test"));
}
